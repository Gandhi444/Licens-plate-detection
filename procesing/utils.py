...
def letter_clasification(letter,low1,low2,low3):
    #lines=cv.HoughLinesP(letter, 1, np.pi / 180, 25, minLineLength=20, maxLineGap=50)
    lines=cv.HoughLinesP(letter, 1, np.pi / 180, low1, minLineLength=low2, maxLineGap=low3)
    cdst = cv.cvtColor(letter.copy(), cv.COLOR_GRAY2BGR)
    cdst2=cdst.copy()
    if lines is not None:
        for i in range(0, len(lines)):
            l = lines[i][0]
            cv.line(cdst2, (l[0], l[1]), (l[2], l[3]), (255,0,255), 3, cv.LINE_AA)
    cv2.imshow('letterlines',cdst2)